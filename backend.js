...
// --- 1.1 SECURITY HEADERS (CRITICAL FIX) ---
app.use((req, res, next) => {
    // 1. COMPLETELY REMOVE restrictive headers that block PhonePe
    res.removeHeader("Content-Security-Policy");
    res.removeHeader("X-Content-Security-Policy");
    res.removeHeader("X-WebKit-CSP");
    
    // 2. Allow everything. This is the "Nuclear Option" to ensure nothing is blocked.
    // We explicitly allow 'blob:' and 'data:' which are required for PhonePe's worker scripts.
    res.setHeader(
...
const getEmailTemplate = (type, data) => {
    const { name, regId, eventName, dept, teamName, txId, date, coupon, link } = data;
    
    const logoUrl = "https://res.cloudinary.com/dpz44zf0z/image/upload/v1764605760/logo_oeso2m.png"; // Your Logo

    // TEMPLATE 1: REGISTRATION CONFIRMED (Sent when Payment Verified)
    if (type === 'REGISTER_CONFIRMED') {
        let teamHtml = '';
        if (data.teamMembers && Array.isArray(data.teamMembers) && data.teamMembers.length > 0) {
            teamHtml = `<p style="margin: 8px 0; color: #333;"><strong>Team Members:</strong> ${data.teamMembers.map(m => m.name).join(', ')}</p>`;
        }
        ...
        <div style="font-family: 'Segoe UI', Arial, sans-serif; padding: 20px; background-color: #fff; max-width: 600px; margin: 0 auto; border: 1px solid #eee;">
            ...
            <h2 style="color: #00d2ff; margin-bottom: 20px; font-weight: bold; text-align: center; font-size: 26px;">LAKSHYA 2K26</h2>
            <p style="font-size: 15px; color: #333;">Dear Participant,</p>
            <p style="font-size: 15px; color: #333;">Your registration for <strong>${eventName}</strong> is confirmed!</p>
            
            <div style="background-color: #f0faff; padding: 25px; border-radius: 4px; margin: 25px 0; border-left: 4px solid #00d2ff;">
                <p style="margin: 8px 0; color: #333;"><strong>Registration ID:</strong> ${regId}</p>
                <p style="margin: 8px 0; color: #333;"><strong>Event:</strong> ${eventName}</p>
                <p style="margin: 8px 0; color: #333;"><strong>Department:</strong> ${dept}</p>
                ${teamName ? `<p style="margin: 8px 0; color: #333;"><strong>Team Name:</strong> ${teamName}</p>` : ''}
                ${teamHtml}
                <p style="margin: 8px 0; color: #333;"><strong>Payment Status:</strong> <strong style="color: #4CAF50;">PAID & VERIFIED</strong></p>
            </div>
            <p style="margin-top: 30px; color: #333;">Best Regards,<br>Team LAKSHYA</p>
        </div>`;
    } 
    
    // TEMPLATE 2: PAYMENT RECEIPT (Sent when Payment Verified)
    if (type === 'PAYMENT_RECEIPT') {
        ...
        <div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #e0e0e0; border-radius: 8px; overflow: hidden; background-color: #fff;">
            ...
            <div style="background-color: #00d2ff; padding: 25px; text-align: center;">
                 <h2 style="color: #ffffff; margin: 0; font-size: 22px; font-weight: bold; text-transform: uppercase; letter-spacing: 1px;">PAYMENT SUCCESSFUL</h2>
            </div>
            <div style="padding: 30px;">
                <p style="font-size: 16px; color: #333;">Dear <strong>${name}</strong>,</p>
                <p style="font-size: 14px; color: #555;">We have received your payment for <strong>${eventName}</strong>.</p>
                
                <div style="background-color: #fcfcfc; padding: 20px; border-left: 5px solid #4CAF50; margin: 25px 0; border-radius: 2px;">
                    <p style="margin: 8px 0; font-size: 14px; color: #333;"><strong>Transaction ID:</strong> ${txId}</p>
                    <p style="margin: 8px 0; font-size: 14px; color: #333;"><strong>Date:</strong> ${date}</p>
                    ${coupon ? `<p style="margin: 8px 0; font-size: 14px; color: #00d2ff;"><strong>Coupon Applied:</strong> ${coupon}</p>` : ''}
                    <p style="margin: 8px 0; font-size: 12px; color: #999;">(Includes Platform Fee)</p>
                </div>

                <div style="text-align: center; margin-top: 35px;">
                    <a href="${link}" target="_blank" style="background-color: #3a7bd5; color: white; padding: 14px 30px; text-decoration: none; border-radius: 6px; font-weight: bold; font-size: 15px;">Download Receipt</a>
                </div>
            </div>
        </div>`;
    }
...
app.post('/api/payment/verify', isAuthenticated('participant'), async (req, res) => {
    try {
        const { merchantTransactionId, registrationIds, couponCode, registrationAmounts } = req.body;
        
        if (!merchantTransactionId) return res.status(400).json({ error: "Missing Transaction ID" });

        const client = initPhonePeClient();

        // 1. Check Status using SDK
        const response = await client.getOrderStatus(merchantTransactionId);

        // 2. Verify State
        if (response.state === 'COMPLETED') {
            const providerRefId = response.paymentInstrument ? response.paymentInstrument.pgTransactionId : "N/A";
                ...
                (async () => {
                    try {
                        for (const regId of registrationIds) {
                            ...
                            const receiptHtml = getEmailTemplate('PAYMENT_RECEIPT', {
                                name: userName, eventName: eventTitle, txId: providerRefId, 
                                date: new Date().toLocaleDateString(), coupon: couponCode,
                                link: `${req.protocol}://${req.get('host')}/receipt-view?id=${regId}`
                            });
                            await sendEmail(reg.studentEmail, `Payment Receipt: ${eventTitle}`, receiptHtml);
                        }
                    } catch (emailErr) { console.error("Email Error:", emailErr); }
                })();

            } else {
                res.json({ status: 'success', warning: 'No registrations to update' });
            }
        } else {
            res.status(400).json({ status: 'failed', message: 'Payment Pending or Failed', state: response.state });
        }
    } catch (err) {
        console.error("SDK Verify Error:", err.message);
